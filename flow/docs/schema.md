...
- §3.2.2 存储与数据
- §3.2.3 数据流转
- §3.2.4 接口设计（Apifox 索引表）
- §4 上线 checkList（§4.1 服务-分支、§4.2 SQL/配置）
...
| 实现类/文件级设计 | 服务 OpenSpec `design.md` |
| spec 拆分、验收标准、c7 测试 | `概要设计.md` |
| DDL 全文 | `发版记录.md` |
| spec 完成勾选 | `task.md`（仅 report 写入） |

### 10.4 禁止内容

- spec 名（`c{n}-*`）、`$flow-codex-*`、Flow 测试分层、审核返修记录
- Java/.vue/.js 文件改动表

...
function Write-Grant([string]$Path, [string]$Change, [string]$Previous, [string]$Ceiling, [string]$Test, [string]$Sut, [string]$Config='config-a') {
    @{schemaVersion=1;grantedBy='user';requestRef='fixture-user-turn';requestText='Proceed through the explicitly named test phase';changeName=$Change;previousCeiling=$Previous;ceiling=$Ceiling;testRevision=$Test;sutRevision=$Sut;harnessRevision=$harness;configurationFingerprint=$Config} | ConvertTo-Json | Set-Content -LiteralPath $Path -Encoding utf8
}
...
try {
    ...
    $grantPath = Join-Path $root 'initial-grant.json'
    Write-Grant $grantPath 'sample-change' 'none' 'execution' $fixture.design $sutRevision $config
    Assert-Controller { & $controller initialize -ReportPath $grantPath -StatePath $state -ChangeName sample-change -SystemTestRepo $system -SutRepo $sut -TestBaselineRevision $fixture.baseline -TestRevision $fixture.design -SutRevision $sutRevision -HarnessRevision $harness -HarnessRoot $harnessRoot -HarnessCertificationPath $harnessCertification -ConfigurationFingerprint $config -Authorization execution }
    ...
    $beforeResultGrant = Get-Content $state -Raw | ConvertFrom-Json
    Write-Grant $grantPath 'sample-change' 'execution' 'result' $implementationRevision $sutRevision $config
    Assert-Controller { & $controller grant-authorization -StatePath $state -TestRevision $implementationRevision -SutRevision $sutRevision -HarnessRevision $harness -ConfigurationFingerprint $config -Authorization result -ReportPath $grantPath }
    $afterResultGrant = Get-Content $state -Raw | ConvertFrom-Json
    if (($beforeResultGrant.runs | ConvertTo-Json -Depth 16) -ne ($afterResultGrant.runs | ConvertTo-Json -Depth 16) -or $beforeResultGrant.phase -ne $afterResultGrant.phase) { throw 'result authorization altered recorded run or phase' }
    ...
    $secretRepo = Join-Path $root 'secret-system'; $secretSut = Join-Path $root 'secret-sut'; $secretFixture = New-GitFixture $secretRepo 'safe-change'; $secretSutFixture = New-GitFixture $secretSut 'safe-sut-change'; $secretState = Join-Path $root 'secret-state.json'; $secretReport = Join-Path $root 'verifier-input.json'; Write-VerifierReport $secretReport 'design' 'verifier-a' $secretFixture.design $secretSutFixture.design $harness $config 'password=must-not-enter-state'; Assert-Controller { & $controller initialize -StatePath $secretState -ChangeName safe-change -SystemTestRepo $secretRepo -SutRepo $secretSut -TestBaselineRevision $secretFixture.baseline -TestRevision $secretFixture.design -SutRevision $secretSutFixture.design -HarnessRevision $harness -HarnessRoot $harnessRoot -HarnessCertificationPath $harnessCertification -ConfigurationFingerprint $config -Authorization design }; Assert-Controller { & $controller record-verifier -StatePath $secretState -VerifyMode design -TestRevision $secretFixture.design -SutRevision $secretSutFixture.design -HarnessRevision $harness -ConfigurationFingerprint $config -ReportPath $secretReport -VerifierId verifier-a } $false 'ERROR_SECRET_INPUT' 'secret-summary'
    ...
    # A real runner failure must become terminal BLOCKED for the current revision.
    $runFail = New-LeasedCase 'runner-fail' 'runner-fail-change'; $runFailRevision = Commit-Implementation $runFail.repo 'runner-fail-change'; $runFailDiff = Get-DiffFixture $runFail.repo $runFail.fixture.design $runFailRevision
    $runFailScope = Join-Path $root 'runner-fail-scope.json'; $runFailResult = Join-Path $root 'runner-fail-implementation.json'; $runFailImplementationReport = Join-Path $root 'runner-fail-verify-implementation.json'; $runFailEnvironmentReport = Join-Path $root 'runner-fail-verify-environment.json'; $runFailResultReport = Join-Path $root 'runner-fail-verify-result.json'; $runFailEvidence = Join-Path $root 'runner-fail-evidence'; New-Item -ItemType Directory -Path $runFailEvidence -Force | Out-Null
    @{ result='PASS'; baselineRevision=$runFail.fixture.design; currentRevision=$runFailRevision; repository=$runFail.repo; changedFiles=$runFailDiff.changedFiles; diffHash=$runFailDiff.diffHash } | ConvertTo-Json | Set-Content $runFailScope -Encoding utf8
    @{ result='PASS'; testRevision=$runFailRevision; implementationBaseRevision=$runFail.fixture.design } | ConvertTo-Json | Set-Content $runFailResult -Encoding utf8
    Assert-Controller { & $controller accept-result -StatePath $runFail.state -ProposedTestRevision $runFailRevision -SutRevision $runFail.sutRevision -HarnessRevision $runFail.harness -ConfigurationFingerprint $runFail.config -ReportPath $runFailResult -ScopeGuardReportPath $runFailScope }
    Write-VerifierReport $runFailImplementationReport 'implementation' 'verifier-a' $runFailRevision $runFail.sutRevision $runFail.harness $runFail.config 'implementation passed' $runFail.fixture.design
    Assert-Controller { & $controller record-verifier -StatePath $runFail.state -VerifyMode implementation -TestRevision $runFailRevision -SutRevision $runFail.sutRevision -HarnessRevision $runFail.harness -ConfigurationFingerprint $runFail.config -ReportPath $runFailImplementationReport -VerifierId verifier-a }
    Write-VerifierReport $runFailEnvironmentReport 'environment' 'verifier-a' $runFailRevision $runFail.sutRevision $runFail.harness $runFail.config
    Assert-Controller { & $controller record-verifier -StatePath $runFail.state -VerifyMode environment -TestRevision $runFailRevision -SutRevision $runFail.sutRevision -HarnessRevision $runFail.harness -ConfigurationFingerprint $runFail.config -ReportPath $runFailEnvironmentReport -VerifierId verifier-a }
    Assert-Controller { & $controller next -StatePath $runFail.state } $true 'next: RUN_ONCE' 'next-before-run'
    Assert-Controller { & $controller start-run -StatePath $runFail.state -TestRevision $runFailRevision -SutRevision $runFail.sutRevision -HarnessRevision $runFail.harness -ConfigurationFingerprint $runFail.config }
    Assert-Controller { & $controller next -StatePath $runFail.state } $true 'next: AWAIT_RUN_RESULT' 'next-during-run'
    Assert-Controller { & $controller record-run -StatePath $runFail.state -RunResult fail -EvidencePath $runFailEvidence -TestRevision $runFailRevision -SutRevision $runFail.sutRevision -HarnessRevision $runFail.harness -ConfigurationFingerprint $runFail.config }
    Assert-Controller { & $controller next -StatePath $runFail.state } $true 'next: BLOCKED' 'next-after-run-fail'
    Write-VerifierReport $runFailResultReport 'result' 'verifier-a' $runFailRevision $runFail.sutRevision $runFail.harness $runFail.config
    ...
    # A cleaned startup/configuration failure may repair only this change's test infrastructure and must keep SUT/harness/config locked.
    $infraFail = New-LeasedCase 'infra-fail' 'infra-fail-change'; $infraRevision = Commit-Implementation $infraFail.repo 'infra-fail-change'; $infraDiff = Get-DiffFixture $infraFail.repo $infraFail.fixture.design $infraRevision
    $infraScope = Join-Path $root 'infra-fail-scope.json'; $infraResult = Join-Path $root 'infra-fail-implementation.json'; $infraImplementationReport = Join-Path $root 'infra-fail-verify-implementation.json'; $infraEnvironmentReport = Join-Path $root 'infra-fail-verify-environment.json'; $infraEvidence = Join-Path $root 'infra-fail-evidence'; New-Item -ItemType Directory -Path $infraEvidence -Force | Out-Null
    @{ result='PASS'; baselineRevision=$infraFail.fixture.design; currentRevision=$infraRevision; repository=$infraFail.repo; changedFiles=$infraDiff.changedFiles; diffHash=$infraDiff.diffHash } | ConvertTo-Json | Set-Content $infraScope -Encoding utf8
    @{ result='PASS'; testRevision=$infraRevision; implementationBaseRevision=$infraFail.fixture.design } | ConvertTo-Json | Set-Content $infraResult -Encoding utf8
    Assert-Controller { & $controller accept-result -StatePath $infraFail.state -ProposedTestRevision $infraRevision -SutRevision $infraFail.sutRevision -HarnessRevision $infraFail.harness -ConfigurationFingerprint $infraFail.config -ReportPath $infraResult -ScopeGuardReportPath $infraScope }
    Write-VerifierReport $infraImplementationReport 'implementation' 'verifier-a' $infraRevision $infraFail.sutRevision $infraFail.harness $infraFail.config
    Assert-Controller { & $controller record-verifier -StatePath $infraFail.state -VerifyMode implementation -TestRevision $infraRevision -SutRevision $infraFail.sutRevision -HarnessRevision $infraFail.harness -ConfigurationFingerprint $infraFail.config -ReportPath $infraImplementationReport -VerifierId verifier-a }
    Write-VerifierReport $infraEnvironmentReport 'environment' 'verifier-a' $infraRevision $infraFail.sutRevision $infraFail.harness $infraFail.config
    Assert-Controller { & $controller record-verifier -StatePath $infraFail.state -VerifyMode environment -TestRevision $infraRevision -SutRevision $infraFail.sutRevision -HarnessRevision $infraFail.harness -ConfigurationFingerprint $infraFail.config -ReportPath $infraEnvironmentReport -VerifierId verifier-a }
    Assert-Controller { & $controller start-run -StatePath $infraFail.state -TestRevision $infraRevision -SutRevision $infraFail.sutRevision -HarnessRevision $infraFail.harness -ConfigurationFingerprint $infraFail.config }
    Assert-Controller { & $controller record-run -StatePath $infraFail.state -RunResult fail -FailureCategory TEST_HARNESS -EvidencePath $infraEvidence -TestRevision $infraRevision -SutRevision $infraFail.sutRevision -HarnessRevision $infraFail.harness -ConfigurationFingerprint $infraFail.config }
    $infraFixtureDir = Join-Path $infraFail.repo 'changes/infra-fail-change/fixtures'; New-Item -ItemType Directory -Path $infraFixtureDir -Force | Out-Null
    Set-Content -LiteralPath (Join-Path $infraFixtureDir 'local-overlay.yml') -Value 'spring.main.allow-bean-definition-overriding: true' -Encoding utf8
    Invoke-Git $infraFail.repo @('add', '.') | Out-Null; Invoke-Git $infraFail.repo @('commit', '--quiet', '-m', 'repair local test overlay') | Out-Null
    $infraRetryRevision = (Invoke-Git $infraFail.repo @('rev-parse', 'HEAD') | Select-Object -First 1).Trim(); $infraRunnerReport = Join-Path $infraEvidence 'runner-result.json'
    @{ status='FAIL'; classification='TEST_HARNESS'; cleanup=@{ succeeded=$true; retainedState=$false } } | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $infraRunnerReport -Encoding utf8
    Assert-Controller { & $controller retry-test-infra-failure -StatePath $infraFail.state -ProposedTestRevision $infraRetryRevision -SutRevision $infraFail.sutRevision -HarnessRevision $infraFail.harness -ConfigurationFingerprint $infraFail.config -ReportPath $infraRunnerReport }
    $infraRetryState = Get-Content -LiteralPath $infraFail.state -Raw | ConvertFrom-Json
    if ($infraRetryState.phase -ne 'TEST_IMPLEMENTED' -or $infraRetryState.revisions.test -ne $infraRetryRevision -or $infraRetryState.scopeVerification.kind -ne 'test-infrastructure-repair') { throw 'test infrastructure retry did not preserve locks and re-enter implementation verification' }

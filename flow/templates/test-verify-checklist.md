...
- **TD.10 ERROR**：manifest 未引用唯一 generated sidecar，sidecar 未绑定 controller 初始 testBaseline，或 Y/N、count、report class、filter、evidence、failureObservability 与 source/baseline revision/hash 漂移；实际设计提交仍由 controller 独立校验。
...
- **TI.8 ERROR**：sidecar、报告类/计数、Y/N、filter、evidence index 或 failureObservability 不是由同一 testBaseline/source hash 派生，controller 当前实现 revision 漂移，或普通/外部证据陈旧。

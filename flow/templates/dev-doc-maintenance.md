...
| §3.2.4 接口设计 | 路径 + **恰好一个服务（repo）** + Apifox 链接 + 变更类型 + 要点；BFF 对外 HTTP 归 BFF 服务；**禁止完整 JSON**；不写 c ID（c↔repo 在概要设计 Spec 矩阵维护） |
...
| 阶段 | 对 `开发文档.md` 的动作 |
|------|-------------------------|
| design（根） | §1–§2 从需求填写；§3.2.1–3.2.3 **摘要/占位**；§3.2.4 接口表骨架（每行一个 repo，与 Spec 矩阵 owning spec 一致；Apifox 列「待录入」）；§4.1 填分支；§4.2 留空 |

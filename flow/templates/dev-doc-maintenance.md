...
| §3.2.4 接口设计 | 路径 + **恰好一个可部署服务** + Apifox 链接 + 变更类型 + 要点；**只列本次新增/修改**（不写无变更/不变/现网行）；BFF 对外 HTTP 归 BFF 服务；**禁止完整 JSON**；不写 c ID |
...
| 阶段 | 对 `开发文档.md` 的动作 |
|------|-------------------------|
| design（根） | §1–§2 从需求填写；§3.2.1–3.2.3 **摘要/占位**；§3.2.4 **只列本次新增/修改接口**（每行一个**可部署服务**；Apifox 列「待录入」）；§4.1 按可部署单元填分支（多模块仓拆行）；§4.2 留空或「无」；§4.3 写业务验收要点（非测试清单）；另产出 `操作链路.md`（见 `操作链路.md.tmpl`） |

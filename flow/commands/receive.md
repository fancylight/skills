...
5. **边界确认（强制）**
   ...
   在创建 TodoList 之前，agent 必须向用户声明：
   > "我的工作是 **{service_name}** 范围内的开发。如涉及其他服务，我会暂停并请求根 agent 协调。"

   同时读取 `.flow/config.yaml` 的 `constraints` 部分（如有），将以下约束告知用户：
   - 禁止修改其他服务代码
   - 禁止擅自变更接口契约
   - 数据库结构变更需显式确认
...
6. **创建 TodoList**
   ...
   **blocked by 硬约束**：
   - 如有 `blocked by` 依赖且依赖服务尚未完成（api.md 不可用），**只允许进入设计阶段，禁止编码实现**。
   - agent 应向用户说明："{依赖服务} 尚未完成，当前只能进行设计，不能编码。"
...
- **如有 `blocked by` 依赖且依赖未完成，只允许设计阶段，禁止编码**
- 如根目录无活跃需求，提示用户联系根 agent 创建需求
- 读取 onboarding.md 后按其中的规范执行后续工作
- **必须执行边界确认步骤**，向用户声明工作范围
- 如用户要求修改其他服务代码，拒绝执行并提示联系根 agent
...
- 新增 flow-codex-sites：Edge 扩展优先，跨 Flow 阶段复用个人平台／操作手册，普通操作不创建 feedback。
- 个人资料独立于安装目录；支持成功后自动补齐、局部重定位、动态租户菜单和 GBP 多模块目录。旧 CDP 文档作为兼容资料。
- 安装测试覆盖重复安装、个人数据保留和回退边界。首批真实验证范围及登录缺口见 docs/sites-delivery-20260921.md，不以入口登记冒充操作通过。

### GLM 项目环境准备与运行前接入（2026-09-21）

- 新增项目 Docker 允许清单与 status/ensure 入口，仅按本次依赖复用或启动既有实例；禁止自动创建替代容器。
- flow-test 在静态预检前、新版 runtime 在业务投递前执行项目环境检查，保留原预算和配置中心/服务就绪检查；报告具体修复动作。
- 修正旧运行契约将探针失败等同用户授权不足的规则，保留真实权限边界。
- GLM 清单不接受私建 Kafka/huawei 容器；旧 standalone runner 升级及真实 Kafka 配置修复不冒称已完成。

### Added

- 测试 design 至交付的追加式计时接口，支持阶段、暂停恢复、介入原因、重复事件去重与中断未分类；不影响 controller 授权、预算或测试 PASS。
- 业务用例生成总览与具名明细表，兼容原字符串源；runner 收集实际断言埋点，无埋点时保持未知。

### Changed

- 范围与 controller 拒绝补充具体动作及恢复指引；移除测试 skills 的逐次编辑租约检查和探针失败一律等待人工要求。
- 业务仓无关文件可通过精确内容与构建影响证据排除，禁止用清理全仓解锁。静态环境预检明确不代表运行就绪，已有资源保留运行时身份核验后复用。

### Fixed

- 自测不再假定临时目录含空格；Maven 参数检查核对完整项目路径。自定义 ArtifactRoot 的报告与认证统一引用实际证据位置，保留隔离与证据哈希校验。

- 统一业务/测试 worktree 的 rebind 入口，兼容无 execution 的旧状态；复用既有整理并继续测试授权，同步当前 harness 路径，保留历史/预算并使旧结果失效，补齐迁移后继续审核与版本接纳的回归。

- 允许现有业务及测试 worktree 位于编排根目录外，默认不创建第三个运行副本；保留精确版本和仓内脚本/配置边界校验。revise 可核实同仓身份后切换主仓绑定，保留历史与预算。

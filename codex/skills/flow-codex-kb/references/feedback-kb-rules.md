...
`flow-codex-kb feedback/{feedback-id}` 专用。主输入：`.flow/feedback/{id}/调查报告.md`。
...
3. 是否为 **CDP / 运维手法**（鉴权、查库步骤、脚本）？→ 进 `{root}/.flow/cdp` playbook（见 `flow-codex-feedback/references/cdp.md`），**不进 KB**

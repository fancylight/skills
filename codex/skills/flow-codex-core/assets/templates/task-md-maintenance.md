...
3. 不影响 spec 条目和开发顺序
4. 提示用户使用 `$flow-codex-assign <service>` 派发 hotfix（assign 识别 hotfix 条目，告诉执行 agent 跳过设计阶段用户评审）

**执行 agent 编码**：走 assign → receive（识别 type=hotfix → 跳过用户设计评审）→ apply（正常编码循环）→ report

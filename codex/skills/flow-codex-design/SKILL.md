...
6. 创建或修复 `.flow/changes/<change_name>/概要设计.md`、`开发文档.md`、`task.md`、`发版记录.md` 和 **`操作链路.md`**。
   ...
   - 开发文档各章节的设计阶段内容，按 `dev-doc-maintenance.md` §7 执行：需求与业务验收明确，实现章节先摘要/占位，接口只列本次新增/修改并按步骤 5 填实际 Apifox 链接或待录入；服务按可部署单元登记。禁止项和自包含要求以该模板为唯一来源。
   - §3.2.4 每条新增/修改接口必须在操作链路中找到调用方；缺失时质疑接口必要性，不为通过检查编造调用方。

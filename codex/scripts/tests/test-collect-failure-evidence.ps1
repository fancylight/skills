...
try {
    ...
    '{"failureObservability":[{"scenarioId":"AC-1-S1","testClass":"SampleTest","testMethod":"fails","category":"FIXTURE_ASSERTION","certainty":"confirmed"}]}' |
    ...
    if ($failureReport -notmatch 'AC-1-S1' -or $failureReport -notmatch 'FIXTURE_ASSERTION' -or $failureReport -notmatch '<redacted>') {

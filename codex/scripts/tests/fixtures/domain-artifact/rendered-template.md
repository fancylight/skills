...
> This file is a rendered domain-model.md.tmpl fixture.
>
> 当前状态：`DOMAIN_DRAFT`。本文件不是概要设计、OpenSpec 或技术方案；未经独立 domain verify，不得据此创建或修改方案产物。
...
> 每条事实必须有稳定 Fact ID、精确定义、生效条件、不生效条件或反例、可定位证据和影响的决策点。不得把当前概要设计、实现设想或 agent 推断作为事实证据。

| Fact ID | 概念 | 精确定义 | 生效条件 | 不生效条件/反例 | 证据 | 影响 Decision ID |
|---|---|---|---|---|---|---|
| DF-001 | record identity | account plus code identifies one record | account and code are both present | null account does not identify a record | EV-001 | DP-001 |

## 表与字段语义

| 表/接口字段 | 业务语义 | 允许值或类型 | 空值/默认值语义 | 证据 Fact ID |
|---|---|---|---|---|
| record.account | owning account | Map<String, Long> | null is invalid | DF-001 |
...
证据等级：E1（用户明确裁决、已批准需求、权威 KB）、E2（当前代码、数据库约束、接口定义、稳定历史 change）、E3（脱敏样例、日志、已有测试）、E4（命名、注释、agent 推断）。
...
- Request domain verify before solution artifacts.

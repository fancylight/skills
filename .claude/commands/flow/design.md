...
- 根模式：`验收标准` 是必须章节，不可省略
- 根模式：设计过程必须与用户交互，关键决策点（服务选择、接口契约、验收标准）必须用 AskUserQuestion 确认
- 子模式：spec 工作目录命名必须为 `c{序号}-{kebab-case描述}`
- 子模式：design.md 必须包含 `## 单元测试计划` 章节，列出所有接口的测试场景
- 子模式：评审只检查与概要设计/task.md 的一致性，不评价方案优劣

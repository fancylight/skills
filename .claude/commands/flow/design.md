...
4.6 **生成 `开发文档.md`**
   ...
   填充策略（design 阶段只写**骨架**，禁止照搬概要设计）：
   - §1 需求文档：询问用户需求文档链接，如无则跳过
   - §2 需求分析：从步骤 2 的需求分析结果填入
   - §3.2.1–§3.2.3：业务规则/存储/流转写**摘要或占位**；禁止复制概要设计中的消费点表、文件清单
   - §3.2.4 接口设计：从步骤 2.5 Apifox 查询填路径与 Apifox 列（无则「待录入」）；**禁止 JSON**
   - §3.1 前端（可选）：仅模块/页面级，禁止文件路径清单
   - §4.1 服务-分支：从 task.md services 填充；§4.2 留空，由 `/flow:report` 补充

...
3. **进入简化编码循环**（跳过阶段一设计）
   ...
   b. 实现修复代码
   c. 内联启动审核 agent，传入以下 prompt：

      ```
      你是代码审核 agent。检查修复代码是否与 fix.md 中的修复方案一致。
      ...
      审核步骤：
      1. 读取 fix.md，提取"修复方案"章节中描述的修改内容
      2. 读取变更代码文件
      3. 检查：代码修改是否精确对应 fix.md 描述的修复方案，没有引入方案之外的改动

      输出格式：
      ## 审核结果：✅ 通过 / ❌ 驳回
      ### 检查项
      - [✅/❌] 修复范围：{代码是否只修改了 fix.md 描述的内容}
      - [✅/❌] 修复正确性：{修改是否符合 fix.md 的根因分析}
      ### 问题详情（仅驳回时）
      1. {文件}:{行号} — {问题描述}
      ```
